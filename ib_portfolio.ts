...
export default class IBPortfolioManager {
  ...
  constructor(private clientId: number = 0, private port: number, private host: string = '127.0.0.1') {
    this.ib = new IBApi({ clientId: this.clientId, host: this.host, port: this.port });
  }
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.ib.on(EventName.error, (err: Error, code: ErrorCode, reqId: number) => {
        console.error(`Connection Error: ${err.message} - Code: ${code} - ReqId: ${reqId}`);
  ...
  setupMarketDataHandlers(): void {
    this.ib.on(EventName.tickPrice, (reqId, tickType, price, attrib) => {
      ...
      if (contract && contract.symbol) {
        ...
        console.log(`Updated market data for ${symbol}: ${JSON.stringify(this.marketData[symbol])}`);
      }
    });
    ...
    this.ib.on(EventName.tickSize, (reqId, tickType, size) => {
      ...
      if (contract && contract.symbol) {
        ...
        console.log(`Updated size data for ${symbol}: ${JSON.stringify(this.marketData[symbol])}`);
      }
    });
  }
  ...
  async disconnect(): Promise<void> {
    return new Promise(resolve => {
      ...
      this.ib.on(EventName.disconnected, () => {
        console.log("Disconnected from IB API."); resolve();
      });
    });
  }
